...
def send_message(chat_id: int, message: str) -> bool:
    """
    Отправляем сообщение пользователю Telegram
    """
    payload = {
        'chat_id': chat_id,
        'text': message
...
def send_audio(chat_id: int, url: str, caption: str = '') -> bool:
    """
    Отправляем аудио файл пользователю Telegram
    """
    payload = {
        'chat_id': chat_id,
        'audio': url
    ...
    headers = {'Content-Type': 'application/json'}
    response = requests.request('POST', f'{BASE_URL}/sendAudio', json=payload, headers=headers)
    return response.status_code == 200 and response.json().get('ok')

def set_webhook(url: str, secret_token: str = '') -> bool:
    """
    Устанавливаем URL в качестве вебхука для получения всех входящих сообщений
    """
    payload = {'url': url}
    if secret_token:
...
def get_file_path(file_id: str) -> dict:
    ...
    if response.status_code == 200:
        return {'status': 1, 'file_path': response.json()['result']['file_path']}
    return {'status': 0, 'file_path': ''}

def save_file_and_get_local_path(file_path: str) -> dict:
    """
    Сохраняем файл и получаем локальный путь к файлу
    """
    url = f'{BASE_URL}/file/bot{config.TOKEN}/{file_path}'
    response = requests.request('GET', url)
    extension = file_path.split('.')[-1]
    file_id = uuid.uuid1()
    file_name = f'{file_id}.{extension}'
    local_file_path = os.path.join(config.OUTPUT_DIR, file_name)
    if response.status_code == 200:
    ...
    return {'status': 0, 'local_file_path': '', 'file_name': '', 'file_id': '', 'extension': ''}
